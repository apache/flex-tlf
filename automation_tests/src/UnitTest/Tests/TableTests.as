...
package UnitTest.Tests
{
	...
	public class TableTests extends VellumTestCase
	{
		...
		private static const JapaneseText:String = '<p paddingBottom="0"><span>ApacheのFlex SDKには、人気のAdobeのFlex SDKの進化である。アパッチ Flex SDKには、簡単にフラッシュを構築するためのアプリケーション開発フレームワークである モバイルデバイス、Webブラウザ、およびデスクトップ·プラットフォームのためのベースのアプリケーション。現在サポートされて るプラットフォームは、次のとおりです</span></p>';
