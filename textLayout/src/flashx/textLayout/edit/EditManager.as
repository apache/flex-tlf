...
package flashx.textLayout.edit
{
	...
	//		CONFIG::debug { assert(captureLevel == 0 || parentOperation != null, "missing parent for nested operation"); }
		...
		public function deleteNextCharacter(operationState:SelectionState = null):void
		{
			...
			{
				//Fix bug #3024299 - [UBS] Delete key does not delete a bullet level other than the last one - Jin Huang
				//Simulate behavior of MS Word
				...
				var cursorAtEnd:Boolean = false;
				if(!(parent is TextFlow))
				{
					var span:SpanElement = leaf as SpanElement;
					if(span)
					{
						if((!(parent is ListItemElement) || parent.parent.getChildIndex(parent) == 0))// if parent is a listItem, it's the first item in the list
						{
							if(operationState.absoluteStart == para.getAbsoluteStart() && parent.getChildIndex(para) == 0 && // cursor is at start of this paragraph AND para is at beginning of parent AND
								span.text == "")//the list item is empty 
							{
								movePara = true;
							}
							else if(span.text != "" && operationState.absoluteStart == para.getAbsoluteStart() + para.textLength - 1 && parent.getChildIndex(para) != parent.numChildren - 1 && para.getNextSibling() is ListElement) 
							{
								leaf = textFlow.findLeaf(operationState.absoluteStart + 1);
								if(leaf) para = leaf.getParagraph();
								if(para) parent = para.parent;
								if(parent)
								{
									movePara = true;
									cursorAtEnd = true;
						...
						else if((!(parent is ListItemElement) || parent.parent.getChildIndex(parent) == parent.parent.numChildren - 1))
						{
							if(span.text == "" && operationState.absoluteStart == para.getAbsoluteStart() + para.textLength - 1 && parent.getChildIndex(para) != parent.numChildren - 1 && para.getNextSibling() is ListElement)
							{
								leaf = textFlow.findLeaf(operationState.absoluteStart + 1);
								if(leaf) para = leaf.getParagraph();
								if(para) parent = para.parent;
				...
				if(movePara)
				{
					...
					beginCompositeOperation();//Composite the two operations as one, then one-time undo() can restore the original state
					
					doOperation(new MoveChildrenOperation(operationState, source, 0, numElementsToMove, target, targetIndex));
					if(!cursorAtEnd)//Cursor at the beginning of a list item
					{
						//Remove the unexpected empty line
						leaf = textFlow.findLeaf(operationState.absoluteStart);
						if(leaf && leaf.text == "")
						{
							para = leaf.getParagraph();
							if(para && para.numChildren == 1)
							{
								parent = para.parent;
								/*It's a trikcy workaround here. To remove the empty line and make the operations undo-able, 
								I put the empty line to the end of textflow. Because deleteTextOperation cannot work here 
								and there is no other operation can remove a specific element for current TLF. - Jin Huang*/
								if(parent)
									doOperation(new MoveChildrenOperation(getSelectionState(), parent, parent.getChildIndex(para), 1, textFlow, textFlow.numChildren));
							}
						}
					}
					else//Cursor at the end of a list item
					{
						leaf = textFlow.findLeaf(operationState.absoluteStart + 1);
						if(leaf && leaf.text == "") 
						{
							para = leaf.getParagraph();
							if(para && para.numChildren == 1)//the original list item is empty
							{
								parent = para.parent;
								//Remove the unexpected empty line
								/*It's a trikcy workaround here. To remove the empty line and make the operations undo-able, 
							...
							doOperation(new DeleteTextOperation(operationState, new SelectionState(textFlow, absoluteStart, absoluteStart + 1, pointFormat), true));
					}
					
					endCompositeOperation(); //end of operations composition
					
				}else				
				{
					var nextPosition:int = NavigationUtil.nextAtomPosition(textFlow, absoluteStart);
					doOperation(new DeleteTextOperation(operationState, new SelectionState(textFlow, absoluteStart, nextPosition, pointFormat), true /* allowMerge for deleteForward */));
				}
			}
			else 
				doOperation(new DeleteTextOperation(operationState, operationState, false /* don't allow merge when deleting by range */));			
		...
		public function deletePreviousCharacter(operationState:SelectionState = null):void
		{
			...
			{	
				...
				//Fix bug #3174671 - [UBS]Create a paragraph, then a bullet above the paragraph. Set cursor at the start of the paragraph 
				//and hit Backspace, the bullet is then removed. Instead, the paragraph is expected to be bulleted - Jin Huang
				if(operationState.absoluteStart == para.getAbsoluteStart() //Cursor is at the beginning of the paragraph
					&& para.getPreviousSibling() is ListElement //A list above the paragraph
					&& leaf.getPreviousLeaf().getParagraph().textLength == 1) //The last list item is empty
				{
					target = leaf.getPreviousLeaf().getParagraph().parent;
					source = para.parent;
					
					beginCompositeOperation();
					doOperation(new MoveChildrenOperation(operationState, source, source.getChildIndex(para), 1, target, target.numChildren));
					doOperation(new DeleteTextOperation(operationState, new SelectionState(textFlow, operationState.absoluteStart - 1, operationState.absoluteStart), true));
					endCompositeOperation();
					return;
				}
